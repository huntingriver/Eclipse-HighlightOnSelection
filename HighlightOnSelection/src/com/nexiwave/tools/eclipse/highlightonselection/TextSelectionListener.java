	...
	private void highlight(IEditorPart editor, ITextViewer viewer, String s) {
		...
		while (m.find()) {
			...
			NEXT_CHAR: for (int i = offset; i < end; i++) {
				...
				//
				newStyleRanges.add(
						new StyleRange(i, end - i, getForegroundColor(text, null), getBackgroundColor(text, null)));
				break;
			}
	...
	private Color getBackgroundColor(StyledText text, StyleRange oldStyleRange) {
		if (bColor == null) {
			...
			bColor = new Color(Display.getCurrent(),
					PreferenceConverter.getColor(store, PreferenceConstants.P_BACKGROUND_COLOR));
		}
		return bColor;
	}
